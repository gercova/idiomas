        ...
        $(document).ready(() => {
            ...
            $('#listado').jtable({
               ...
               defaultSorting: 'Orde date ASC', // ordenado ascendente
                ...
                actions: {
                     listAction: '<?php echo site_url(); ?>mantenimiento/conceptos/lista',
                ...
                fields: {
                    ...
                    $('.edit-row').click(function(e) {
                        ...
                        console.log(id),
                        //window.location.href = `<?php echo site_url(); ?>administrador/conceptos/edit/${id}`;
                        //LoadRecordsButton.click();
  
                        $.ajax({
                                url: base_url + "mantenimiento/conceptos/edit",
                                method: "POST",
                                data:{id:id},
                                dataType: "json"
                        })
                            ...
                            // console.log(result);
                                ...
                                $("#referencia").val(result.referencia).prop("disabled", false);
                                $("#btnGuardar").prop('disabled', false);
                                $("#btn-consultar-dni").prop('disabled', false);
                                $(".modal-header-color").css("background-color", "#28a745");
                                $(".modal-header-color").css("color", "white");
                                $(".modal-title-titulo").text("Editar Referencia");        
                    ...
                    $('.view-row').click(function(e) {
                        ...
                        $.ajax({
                            ...
                            // console.log(result);
                                ...
                                $(".modal-header-color").css("color", "white");
                                $(".modal-title-titulo").text("Datos de la Referencia");         
                                $("#modalCRUD").modal("show");      
                            })
                            .fail(function(jqXHR, textStatus, errorThrown) {
                            });
                    ...
                    $('.delete-row').click(function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                console.log(id)
                                const url = `<?php echo site_url(); ?>mantenimiento/conceptos/delete/${id}`;
                                fetch(url)
                                    .then(res=>res.json())
                                    .then(res => {
                                        console.log(res)
...
$("#form_conceptos").submit(function(e){
   e.preventDefault();  
   // console.log(email);
    id =$.trim($("#id").val());    
    ...
    $.ajax({
        url: "<?php echo base_url(); ?>mantenimiento/conceptos/store",
        type: "POST",
        dataType: "json",
        data: {id:id, descripcion:descripcion, referencia:referencia},
       // data:{guardar},
