        ...
        $(document).ready(() => {
            ...
            $('#listado').jtable({
               ...
               defaultSorting: 'Orde date ASC', // ordenado ascendente
                ...
                actions: {
                     listAction: '<?php echo site_url(); ?>mantenimiento/dias/lista',
                ...
                fields: {
                    ...
                    $('.edit-row').click(function(e) {
                        ...
                        console.log(id),
                        //window.location.href = `<?php echo site_url(); ?>administrador/dias/edit/${id}`;
                        //LoadRecordsButton.click();
  
                        $.ajax({
                                url: base_url + "mantenimiento/dias/edit",
                                method: "POST",
                                data:{id:id},
                                dataType: "json"
                        })
                            ...
                            // console.log(result);
                                ...
                                $(".modal-title-titulo").text("Editar Día");        
                    ...
                    $('.view-row').click(function(e) {
                        ...
                        $.ajax({
                            ...
                            // console.log(result);
                                ...
                                $(".modal-header-color").css("color", "white");
                                $(".modal-title-titulo").text("Datos del Día");         
                                $("#modalCRUD").modal("show");      
                            })
                            .fail(function(jqXHR, textStatus, errorThrown) {
                            });
                    ...
                    $('.delete-row').click(function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                console.log(id)
                                const url = `<?php echo site_url(); ?>mantenimiento/dias/delete/${id}`;
                                fetch(url)
                                    .then(res=>res.json())
                                    .then(res => {
                                        console.log(res)
...
const newRecord = () => {
    $("#form_dias").trigger("reset");
    $(".modal-header-color").css("background-color", "#007bff");
    $(".modal-header-color").css("color", "white");
    $(".modal-title-titulo").text("Nueva Día");            
    $("#modalCRUD").modal("show");        
    id="";
    }

$("#form_dias").submit(function(e){
   e.preventDefault();  
   // console.log(email);
    id =$.trim($("#id").val());    
    ...
    $.ajax({
        url: "<?php echo base_url(); ?>mantenimiento/dias/store",
        type: "POST",
        dataType: "json",
